...
export async function registerCrudRoutes(app: FastifyInstance) {
  ...
  app.post("/clientes", async (req) => {
    ...
    const id = nanoid()
    db.prepare("INSERT INTO clientes (id,companyId,nome,documento,endereco,telefone,email,createdAt) VALUES (?,?,?,?,?,?,?,?)").run(
      id,
      companyId,
      body.nome,
      body.documento,
  ...
  app.put("/clientes/:id", async (req) => {
    ...
    db.prepare("UPDATE clientes SET nome=?, documento=?, endereco=?, telefone=?, email=? WHERE id=? AND companyId=?").run(
      b.nome,
      b.documento,
      b.endereco ?? null,
      b.telefone ?? null,
  ...
  app.post("/produtos", async (req) => {
    ...
    const id = nanoid()
    db.prepare(
      `INSERT INTO produtos (id,companyId,nome,descricao,marca,precoVenda,custo,taxaImposto,modalidadeVenda,estoque,linkRef,custoRef,createdAt)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ).run(
      id,
      companyId,
  ...
  app.put("/produtos/:id", async (req) => {
    ...
    const b = req.body as any
    db.prepare(
      `UPDATE produtos SET nome=?,descricao=?,marca=?,precoVenda=?,custo=?,taxaImposto=?,modalidadeVenda=?,estoque=?,linkRef=?,custoRef=? WHERE id=? AND companyId=?`,
    ).run(
      b.nome,
      b.descricao ?? null,
      b.marca ?? null,
  ...
  app.get("/pedidos", async (req) => {
    ...
    const rows = db.prepare(pedidosQuery).all(companyId) as any[]
  ...
  app.post("/pedidos", async (req) => {
    ...
    const b = req.body as any
    const id = nanoid()
    const numeroRow = db.prepare("SELECT value FROM seqs WHERE key='pedido'").get() as any
    const numero = Number(numeroRow?.value ?? 1)
    const tx = db.transaction(() => {
      db.prepare("INSERT INTO pedidos (id,companyId,numero,data,clienteId,tipo,observacoes) VALUES (?,?,?,?,?,?,?)").run(
        id,
        companyId,
        numero,
        b.data,
      ...
      const ins = db.prepare(
        "INSERT INTO itens_pedido (id,pedidoId,produtoId,quantidade,precoUnitario,custoUnitario,taxaImposto) VALUES (?,?,?,?,?,?,?)",
      )
  ...
  app.put("/pedidos/:id", async (req) => {
    ...
    const b = req.body as any
    const tx = db.transaction(() => {
      db.prepare("UPDATE pedidos SET data=?, clienteId=?, tipo=?, observacoes=? WHERE id=? AND companyId=?").run(
        b.data,
        b.clienteId,
        b.tipo,
        b.observacoes ?? null,
  ...
  app.post("/recebimentos", async (req) => {
    ...
    const b = req.body as any
    const id = nanoid()
    db.prepare("INSERT INTO recebimentos (id,companyId,pedidoId,valor,data,formaPagamento) VALUES (?,?,?,?,?,?)").run(
      id,
      companyId,
      b.pedidoId,
      b.valor,
  ...
  app.put("/recebimentos/:id", async (req) => {
    ...
    const id = (req.params as any).id
    const b = req.body as any
    db.prepare("UPDATE recebimentos SET pedidoId=?, valor=?, data=?, formaPagamento=? WHERE id=? AND companyId=?").run(
      b.pedidoId,
      b.valor,
      b.data,
      b.formaPagamento,
  ...
  app.get("/empresa-config/:empresaId", async (req) => {
    const empresaId = (req.params as any).empresaId
    const row = db.prepare("SELECT json FROM empresa_config WHERE empresaId=?").get(empresaId) as
      | { json: string }
      | undefined
  ...
  app.put("/empresa-config/:empresaId", async (req) => {
    ...
    if (up.changes === 0) db.prepare("INSERT INTO empresa_config (empresaId,json) VALUES (?,?)").run(empresaId, json)
    return { ok: true }
  })
  ...
  app.put("/user-prefs", async (req) => {
    ...
    if (up.changes === 0) db.prepare("INSERT INTO user_prefs (userId,json) VALUES (?,?)").run("default", json)
    return { ok: true }
  })
  ...
  app.post("/linhas-venda", async (req) => {
    ...
    db.prepare(
      ...
      b.dataPedido,
      b.numeroOF ?? null,
      b.numeroDispensa ?? null,
      b.cliente ?? null,
      b.produto ?? null,
      b.modalidade ?? null,
      calc.valorVenda,
      calc.taxaCapitalPerc,
      calc.taxaCapitalVl,
      calc.taxaImpostoPerc,
      calc.taxaImpostoVl,
      calc.custoMercadoria,
      calc.somaCustoFinal,
      calc.lucroValor,
      calc.lucroPerc,
      b.dataRecebimento ?? null,
      b.paymentStatus ?? "PENDENTE",
      b.settlementStatus ?? null,
      b.acertoId ?? null,
      b.cor ?? null,
      new Date().toISOString(),
    )
  ...
  app.put("/linhas-venda/:id", async (req) => {
    ...
    const calc = computeLine(b)
    db.prepare(
      `UPDATE linhas_venda SET companyId=?,dataPedido=?,numeroOF=?,numeroDispensa=?,cliente=?,produto=?,modalidade=?,valorVenda=?,
       taxaCapitalPerc=?,taxaCapitalVl=?,taxaImpostoPerc=?,taxaImpostoVl=?,custoMercadoria=?,somaCustoFinal=?,lucroValor=?,lucroPerc=?,
       dataRecebimento=?,paymentStatus=?,settlementStatus=?,acertoId=?,cor=? WHERE id=?`,
    ).run(
      b.companyId ?? null,
      b.dataPedido,
      b.numeroOF ?? null,
  ...
  app.patch("/linhas-venda/:id/cor", async (req) => {
    ...
    const row = db.prepare("SELECT companyId FROM linhas_venda WHERE id=?").get(id) as { companyId: string } | undefined
  ...
  app.patch("/linhas-venda/:id", async (req) => {
    ...
    if (b.acertoId !== undefined) { updates.push('acertoId=?'); values.push(b.acertoId) }
    if (b.settlementStatus !== undefined) { updates.push('settlementStatus=?'); values.push(b.settlementStatus) }
    if (b.paymentStatus !== undefined) { updates.push('paymentStatus=?'); values.push(b.paymentStatus) }
    if (b.dataRecebimento !== undefined) { updates.push('dataRecebimento=?'); values.push(b.dataRecebimento) }
    if (b.cor !== undefined) { updates.push('cor=?'); values.push(b.cor) }
  ...
  app.get("/acertos", async (req) => {
    ...
    const rows = db.prepare("SELECT * FROM acertos WHERE companyId=? ORDER BY createdAt DESC").all(companyId) as any[]
    // Parse JSON fields
  ...
  app.post("/acertos", async (req) => {
    ...
    db.prepare(`INSERT INTO acertos 
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(
        id, companyId, b.data, b.titulo || null, b.observacoes || null,
        JSON.stringify(b.linhaIds || []), b.totalLucro, b.totalDespesasRateio,
        b.totalDespesasIndividuais, b.totalLiquidoDistribuivel,
        JSON.stringify(b.distribuicoes || []), JSON.stringify(b.despesas || []),
        b.ultimoRecebimentoBanco ? JSON.stringify(b.ultimoRecebimentoBanco) : null,
        b.status || 'aberto', createdAt
      )
  ...
  app.patch("/acertos/:id", async (req) => {
    ...
    if (b.titulo !== undefined) { updates.push('titulo=?'); values.push(b.titulo) }
    if (b.observacoes !== undefined) { updates.push('observacoes=?'); values.push(b.observacoes) }
    if (b.status !== undefined) { updates.push('status=?'); values.push(b.status) }
    if (b.linhaIds !== undefined) { updates.push('linhaIds=?'); values.push(JSON.stringify(b.linhaIds)) }
    if (b.distribuicoes !== undefined) { updates.push('distribuicoes=?'); values.push(JSON.stringify(b.distribuicoes)) }
    if (b.despesas !== undefined) { updates.push('despesas=?'); values.push(JSON.stringify(b.despesas)) }
  ...
  app.post("/despesas-pendentes", async (req) => {
    ...
    db.prepare(`INSERT INTO despesas_pendentes 
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(id, companyId, b.descricao, b.valor, b.tipo, b.participanteId || null, 
           b.status || 'pendente', b.usedInAcertoId || null, createdAt)
  ...
  app.patch("/despesas-pendentes/:id", async (req) => {
    ...
    if (b.descricao !== undefined) { updates.push('descricao=?'); values.push(b.descricao) }
    if (b.valor !== undefined) { updates.push('valor=?'); values.push(b.valor) }
    if (b.tipo !== undefined) { updates.push('tipo=?'); values.push(b.tipo) }
    if (b.participanteId !== undefined) { updates.push('participanteId=?'); values.push(b.participanteId) }
    if (b.status !== undefined) { updates.push('status=?'); values.push(b.status) }
    if (b.usedInAcertoId !== undefined) { updates.push('usedInAcertoId=?'); values.push(b.usedInAcertoId) }
  ...
  app.get("/orcamentos", async (req) => {
    ...
    const rows = db.prepare("SELECT * FROM orcamentos WHERE companyId=? ORDER BY data DESC, numero DESC").all(companyId) as any[]
    return rows.map(row => ({
      ...row,
  ...
  app.post("/orcamentos", async (req) => {
    ...
    // Get next numero
    const seqRow = db.prepare("SELECT value FROM seqs WHERE key = 'orcamento'").get() as { value: number } | undefined
    ...
    db.prepare(`INSERT INTO orcamentos 
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(
        id, companyId, numero, b.data || now, 
        b.cliente?.id || null, b.cliente?.nome || '', b.cliente?.documento || null, 
        b.cliente?.telefone || null, b.cliente?.email || null, b.cliente?.endereco || null,
        JSON.stringify(b.itens || []), b.observacoes || null, now, now
      )
  ...
  app.patch("/orcamentos/:id", async (req) => {
    ...
    db.prepare(`UPDATE orcamentos SET 
      ...
      .run(
        b.cliente?.id || null, b.cliente?.nome || '', b.cliente?.documento || null,
        b.cliente?.telefone || null, b.cliente?.email || null, b.cliente?.endereco || null,
        JSON.stringify(b.itens || []), b.observacoes || null, now, id, companyId
      )
  ...
  app.get("/vale-movimentos/cliente/:clienteId", async (req) => {
    const clienteId = (req.params as any).clienteId
    const companyId = getCurrentCompanyId(req)
    if (!companyId) {
      return []
    }
  ...
  app.get("/vale-saldos", async (req) => {
    ...
    const movimentos = db.prepare("SELECT clienteId, tipo, valor FROM vale_movimentos WHERE companyId=?").all(companyId) as any[]
  ...
  app.post("/outros-negocios", async (req) => {
    ...
    db.prepare(`INSERT INTO outros_negocios 
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(id, companyId, b.pessoa, b.tipo, b.descricao, b.valor, b.data, 
           b.jurosAtivo ? 1 : 0, b.jurosMesPercent || null, 
           JSON.stringify(b.pagamentos || []), createdAt)
  ...
  app.put("/outros-negocios/:id", async (req) => {
    ...
    const current = db.prepare("SELECT * FROM outros_negocios WHERE id=? AND companyId=?").get(id, companyId) as any
    if (!current) {
