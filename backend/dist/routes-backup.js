...
export async function registerBackup(app) {
    app.get("/backup/export", async (req) => {
        ...
        const empresaRow = d.prepare(`SELECT json FROM empresa_config WHERE empresaId=?`).get(companyId);
        const empresa = empresaRow ? JSON.parse(empresaRow.json || '{}') : null;
        const seq = d.prepare(`SELECT value FROM seqs WHERE key='pedido'`).get();
        return {
            version: 1,
    ...
    app.post("/backup/import", async (req) => {
        const body = req.body;
        const merge = String(req.query?.merge ?? "1") !== "0";
        const d = getDb();
        const companyId = getCurrentCompanyId(req);
        if (!companyId) {
            return { error: "No company selected" };
