...
import { withCache, CACHE_KEYS, CACHE_TTL } from "./cache.js";
...
export async function registerDashboardRoutes(app) {
    app.get("/dashboard/totals", async (req) => {
        ...
        return withCache(CACHE_KEYS.dashboardTotals(companyId), CACHE_TTL.DASHBOARD_TOTALS, async () => {
            const db = getDb();
          ...
          WHERE companyId = ?
        `;
            const totals = db.prepare(totalsQuery).get(companyId);
            // Get total received from linhas_venda where dataRecebimento is not null
            const totalRecebido = db.prepare("SELECT COALESCE(SUM(valorVenda), 0) as total FROM linhas_venda WHERE companyId = ? AND dataRecebimento IS NOT NULL").get(companyId);
        ...
        `;
            const pendentes = db.prepare(pendentesQuery).get(companyId);
    ...
    app.get("/dashboard/series", async (req) => {
        ...
        return withCache(CACHE_KEYS.dashboardSeries(companyId), CACHE_TTL.DASHBOARD_SERIES, async () => {
            const db = getDb();
          ...
          ORDER BY month ASC
        `;
            const results = db.prepare(seriesQuery).all(companyId);
    ...
    app.get("/dashboard/summary", async (req) => {
        ...
        const totalClientes = db.prepare("SELECT COUNT(*) as count FROM clientes WHERE companyId = ?").get(companyId);
        // Count total unique orders for the company (using numeroOF as identifier)
        const totalPedidos = db.prepare("SELECT COUNT(DISTINCT numeroOF) as count FROM linhas_venda WHERE companyId = ? AND numeroOF IS NOT NULL").get(companyId);
        // Count pending orders (where dataRecebimento is null or paymentStatus is not 'pago')
        const pedidosPendentes = db.prepare("SELECT COUNT(*) as count FROM linhas_venda WHERE companyId = ? AND (dataRecebimento IS NULL OR paymentStatus != 'pago')").get(companyId);
        return {
    ...
    app.get("/dashboard/alerts", async (req) => {
      ...
      LIMIT 5
    `;
        const alerts = db.prepare(alertsQuery).all(companyId);
