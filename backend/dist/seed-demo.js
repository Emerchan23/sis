...
function ensureCliente(nome) {
    const row = db.prepare(`SELECT id FROM clientes WHERE nome=? AND companyId=?`).get(nome, defaultCompanyId);
    if (row?.id)
        return row.id;
    const id = uid();
...
function ensureProduto(nome, preco) {
    const row = db.prepare(`SELECT id FROM produtos WHERE nome=? AND companyId=?`).get(nome, defaultCompanyId);
    if (row?.id)
        return row.id;
    const id = uid();
...
function addPedido(clienteId, total, diasAtras) {
    const id = uid();
    const emissao = new Date(Date.now() - diasAtras * 24 * 3600_000).toISOString();
    db.prepare(`INSERT INTO pedidos (id, numero, data, clienteId, tipo, observacoes, companyId)
     VALUES (@id,@numero,@data,@clienteId,@tipo,@observacoes,@companyId)`).run({
        id,
    ...
    const recId = uid();
    db.prepare(`INSERT INTO recebimentos (id, pedidoId, valor, data, formaPagamento, companyId)
     VALUES (@id,@pedidoId,@valor,@data,@formaPagamento,@companyId)`).run({
        id: recId,
        pedidoId: id,
        valor: total / 2,
        data: emissao,
